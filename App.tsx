...
import UserRegistrationScreen from "./src/screens/Auth/UserRegistrationScreen";
...
const MainNavigator = ({ userProfile }: { userProfile: UserProfile }) => {
  ...
  const initialRouteForMotorista = needsVehicleRegistration(userProfile)
    ? "CarRegistration"
    : "HomeMotorista";
  ...
  return (
    <AppStack.Navigator
      initialRouteName={
        modoAtualRoot === "motorista" && userProfile.perfil === "motorista"
          ? (initialRouteForMotorista as any)
          : "HomePassageiro"
      }
      screenOptions={{
        headerShown: true,
    ...
    >
      {modoAtualRoot === "passageiro" || userProfile.perfil !== "motorista" ? (
        ...
        <>
          ...
          />
          <AppStack.Screen
            name="DriverEarningsDay"
            component={
              require("./src/screens/Driver/DriverEarningsDayScreen").default
            }
...
const App = () => {
            ...
            if (userDoc.exists()) {
              ...
              if (!userData.modoAtual) {
                try {
                  ...
                  logger.debug("AUTH", "modoAtual padrão salvo (passageiro)", {
                    uid: firebaseUser.uid,
                  });
                } catch (e) {
              ...
              logger.info("AUTH", "Dados do usuário carregados", {
                ...
                needsVehicleRegistration:
                  needsVehicleRegistration(completeUserData),
              });
              ...
              try {
                if (Constants.appOwnership === "expo") {
                  logger.warn(
                    "PUSH_TOKEN",
                    "Executando em Expo Go — pulando registro de push. Use um development build para testar push."
                  );
                } else {
              ...
              logger.warn(
                "AUTH",
                "Usuário sem perfil no Firestore - criando perfil básico"
              );
  ...
  const getCurrentScreen = () => {
    ...
    if (modoAtual === "motorista" && user.perfil === "motorista") {
      ...
      if (needsVehicleRegistration(user)) {
        logger.info(
          "APP",
          "Motorista sem veículo - mostrando UserRegistrationNavigator",
          {
            nome: user.nome,
            perfil: user.perfil,
            motoristaData: user.motoristaData,
        ...
        return <AuthNavigator initialRouteName="CarRegistration" />;
      }

      logger.info(
        "APP",
        "Modo motorista detectado - redirecionando para MainNavigator (Motorista)",
        {
          nome: user.nome,
          perfil: user.perfil,
        }
    ...
    logger.info(
      "APP",
      "Modo passageiro detectado - redirecionando para MainNavigator (Passageiro)",
      {
        nome: user.nome,
        perfil: user.perfil,
      }
    ...
    logger.warn("APP", "Estado indeterminado - mostrando AuthNavigator", {
      perfil: user?.perfil,
    });
  ...
  if (bootstrapError) {
    return (
      <View
        style={[styles.errorContainer, { backgroundColor: theme.whiteAreia }]}
      >
  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        <Text style={{ color: theme.blueBahia, marginTop: 10 }}>
          Carregando Bahia Driver...
        </Text>
      </View>
    );
  }
  ...
  return (
    <NavigationContainer ref={rootNavigationRef}>
      <View style={styles.container}>{getCurrentScreen()}</View>
    </NavigationContainer>
  );
};
