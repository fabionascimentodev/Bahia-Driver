...
export async function createUserWithEmailAndPassword(
  ...
  try {
    logger.info("USER_SERVICE", "Cadastrando novo usuário", {
      email,
      nome,
    ...
    // Small helper: wait for auth.currentUser to be initialized after creating user
    const waitForAuth = async (expectedUid: string, timeout = 3000) => {
    ...
    // Aguarda um pouco para evitar condições de corrida onde o token ainda
    // não foi propagado para as chamadas do Firestore (falha comum em mobile)
    // wait a bit more for client token propagation and force a token refresh so Firestore
    // sees authenticated requests with a valid token. This reduces 'Missing or insufficient permissions' errors.
    await waitForAuth(user.uid, 6000);
    try {
      if (
        auth.currentUser &&
        ...
        logger.debug(
          "USER_SERVICE",
          "Token de ID atualizado antes de gravar Firestore",
          { uid: user.uid }
        );
      }
    } catch (tokenErr) {
      logger.warn(
        "USER_SERVICE",
        "Falha ao atualizar ID token (continuando):",
        tokenErr
      );
    }
    ...
    if (perfil) {
      ...
      // Se o perfil foi fornecido no pre-cadastro (ex: motorista), inicialize o modoAtual
      // para evitar redirecionamentos indesejados para o modo passageiro.
      userData.modoAtual = perfil;
    }

    try {
      // Try to ensure token has propagated before creating profile in Firestore
      await waitForAuth(user.uid, 6000);
      if (
        auth.currentUser &&
        auth.currentUser.uid === user.uid &&
      ...
      logger.success("USER_SERVICE", "Perfil criado no Firestore", {
        perfil: perfil || "pendente",
      });
    } catch (e: any) {
      logger.error("USER_SERVICE", "Falha ao criar perfil no Firestore", e);
      // Se a criação do documento falhar por permissões insuficientes, tente
      // limpar o usuário recém-criado para não deixar contas órfãs.
      ...
      if (
        ...
        try {
          logger.warn(
            "USER_SERVICE",
            "Tentando remover usuário Auth criado devido a falha no Firestore",
            { uid: user.uid }
          );
          // delete the user if it's the current user
          if (auth.currentUser && auth.currentUser.uid === user.uid) {
            await user.delete();
          } else {
            // try signing in as this user briefly to delete (best-effort)
            try {
              await signOut(auth);
            } catch (_) {}
          }
          ...
          logger.error(
            "USER_SERVICE",
            "Falha ao remover usuário Auth após erro no Firestore",
            cleanupErr
          );
        }
      }
...
export async function updateUserProfileType(
  ...
  try {
    logger.info("USER_SERVICE", "Criando/atualizando perfil do usuário", {
      uid,
      perfil,
      nome,
    });
    ...
    // Garantir token pronto antes de gravar para evitar condições de corrida
    try {
      const start = Date.now();
      ...
      logger.warn(
        "USER_SERVICE",
        "Falha ao esperar/refresh token antes de atualizar perfil (continuando)",
        e
      );
    }
    ...
    if (perfil === "motorista") {
      try {
        ...
        logger.warn(
          "USER_SERVICE",
          "Falha ao refresh token antes de criar motoristaData (continuando)",
          e
        );
      }
    ...
    logger.success("USER_SERVICE", "Perfil do usuário salvo com sucesso", {
      perfil,
    });
  } catch (error) {
...
export async function updateDriverAvailability(
  ...
  try {
    logger.info("USER_SERVICE", "Atualizando disponibilidade do motorista", {
      uid,
      status,
    });
 ...
 */
export async function saveDriverVehicleData(
  uid: string,
  ...
  try {
    ...
    if (vehicleData.modelo !== undefined)
      veiculoPayload.modelo = vehicleData.modelo;
    if (vehicleData.placa !== undefined)
      veiculoPayload.placa = vehicleData.placa;
    ...
    if (vehicleData.documentoUrl)
      veiculoPayload.documentoUrl = vehicleData.documentoUrl;
    if (vehicleData.cnhUrl) veiculoPayload.cnhUrl = vehicleData.cnhUrl;
    if (vehicleData.antecedenteFileUrl)
      veiculoPayload.antecedenteFileUrl = vehicleData.antecedenteFileUrl;
    ...
    if (vehicleData.antecedenteFileUrl) {
      updatePayload["motoristaData.antecedenteFileUrl"] =
        vehicleData.antecedenteFileUrl;
    }

    // Aguarda token e atualiza (mitiga erros de permissão na escrita logo após criação do usuário)
    if (
      auth.currentUser &&
      auth.currentUser.uid === uid &&
      typeof auth.currentUser.getIdToken === "function"
    ...
    logger.success("USER_SERVICE", "Dados do veículo salvos com sucesso", {
      placa: vehicleData.placa,
    });
  } catch (error) {
...
export async function uploadVehiclePhoto(
  ...
  try {
    logger.info("USER_SERVICE", "Iniciando upload para Firebase Storage", {
      uid,
      placa,
    });
    ...
    if (
      ...
      const e: any = new Error(
        "Sem permissão para fazer upload. Verifique as regras de segurança do Firebase Storage."
      );
...
export async function checkStoragePermissions(uid: string): Promise<boolean> {
  try {
    ...
    const testRef = ref(
      storage,
      `vehicles/${uid}/test_permission_${Date.now()}.txt`
    );
    ...
    logger.success(
      "USER_SERVICE",
      "Permissões do Storage verificadas com sucesso"
    );
    return true;
  } catch (error) {
    logger.error(
      "USER_SERVICE",
      "Falha na verificação de permissões do Storage",
      error
    );
    return false;
  }
...
export async function uploadUserAvatar(
  ...
  try {
    ...
    if (!authReady) {
      logger.warn(
        "USER_SERVICE",
        "auth.currentUser não estava pronto antes do upload de avatar; continuando mesmo assim",
        { uid, authUid: auth.currentUser?.uid }
      );
    }
    ...
    const metadata = {
      ...
      customMetadata: { owner: uid, uploadedAt: new Date().toISOString() },
    };
    ...
    logger.debug("USER_SERVICE", "Upload concluído no Storage", {
      fullPath: snapshot.metadata.fullPath,
    });
    ...
    try {
      ...
      await setDoc(
        userRef,
        { avatarUrl: downloadURL, updatedAt: new Date() },
        { merge: true }
      );
      logger.success("USER_SERVICE", "avatarUrl salvo no Firestore", {
        uid,
        avatarUrl: downloadURL.substring(0, 80) + "...",
      });
    } catch (e) {
    ...
    logger.success("USER_SERVICE", "Avatar enviado e salvo", {
      uid,
    ...
    if (
      ...
      const e: any = new Error(
        "Sem permissão para fazer upload do avatar. Verifique as regras de segurança do Firebase Storage."
      );
      e.code = "storage-unauthorized";
      throw e;
    }
 ...
 */
export async function uploadCnhPhoto(
  uid: string,
  localUri: string
): Promise<string> {
  try {
    logger.info(
      "USER_SERVICE",
      "Iniciando upload de CNH para Firebase Storage",
      { uid }
    );
    ...
    const maxSize = 5 * 1024 * 1024; // 5MB
    if (blob.size > maxSize)
      throw new Error("A imagem da CNH é muito grande. Máx 5MB");
    ...
    logger.success("USER_SERVICE", "CNH enviada para Storage", {
      uid,
    ...
    if (
      ...
      const e: any = new Error(
        "Sem permissão para fazer upload da CNH. Verifique as regras de segurança do Firebase Storage."
      );
      e.code = "storage-unauthorized";
      throw e;
    }
 ...
 */
export async function uploadAntecedenteFile(
  uid: string,
  localUri: string,
  fileName?: string
): Promise<string> {
  try {
    logger.info(
      "USER_SERVICE",
      "Iniciando upload de arquivo de antecedentes para Firebase Storage",
      { uid }
    );
    ...
    const safeName = fileName
      ...
      : `antecedente_${Date.now()}`;
    const ext =
      (safeName.includes(".") ? safeName.split(".").pop() : "pdf") || "pdf";
    ...
    logger.success(
      "USER_SERVICE",
      "Arquivo de antecedentes enviado para Storage",
      { uid, path: snapshot.metadata.fullPath }
    );
    return downloadURL;
  } catch (error: any) {
    logger.error(
      "USER_SERVICE",
      "Erro ao enviar arquivo de antecedentes",
      error
    );
    const msg =
      (error && (error.message || error.toString())) || "Erro desconhecido";
    if (
      ...
      const e: any = new Error(
        "Sem permissão para fazer upload do arquivo de antecedentes. Verifique as regras de segurança do Firebase Storage."
      );
      e.code = "storage-unauthorized";
      throw e;
    }
...
export async function confirmPhoneSignIn(
  ...
  try {
    ...
    logger.success("USER_SERVICE", "Login por telefone confirmado", {
      uid: user.uid,
    ...
    if (!userDoc.exists()) {
      ...
      logger.success(
        "USER_SERVICE",
        "Perfil criado no Firestore para login por telefone",
        { uid: user.uid }
      );
    } else {
      ...
      if (!data.telefone && user.phoneNumber)
        updates.telefone = user.phoneNumber;
      if (!data.modoAtual) updates.modoAtual = "passageiro";
      if (Object.keys(updates).length > 1) {
        await setDoc(userRef, updates, { merge: true });
        logger.debug(
          "USER_SERVICE",
          "Perfil atualizado com telefone/modoAtual padrão",
          { uid: user.uid }
        );
      }
    }
    ...
    if (
      ...
      throw {
        code: "phone-linked-elsewhere",
        message:
          "Este número de telefone já está associado a outra conta. Tente entrar com telefone ou com o e-mail associado.",
      };
    }
    throw error;
  }
...
export async function linkPhoneToCurrentUser(
  ...
  try {
    if (!auth.currentUser)
      throw new Error("Usuário não autenticado para vincular telefone");
    logger.info("USER_SERVICE", "Vinculando telefone à conta atual", {
    ...
    await setDoc(
      userRef,
      { telefone: linked.user.phoneNumber, updatedAt: new Date() },
      { merge: true }
    );
    logger.success("USER_SERVICE", "Telefone vinculado e salvo no Firestore", {
      uid: auth.currentUser.uid,
      phone: linked.user.phoneNumber,
    });
    ...
    if (
      ...
      throw {
        ...
        message:
          "Este número de telefone já está vinculado a outra conta. Você pode entrar usando o telefone ou contatar o suporte.",
      };
    }
    if (code === "auth/requires-recent-login") {
      throw {
        ...
        message:
          "Para vincular o telefone, é necessária uma autenticação recente. Saia e entre novamente e tente novamente.",
      };
    }
 ...
 */
export async function setModoAtual(
  uid: string,
  modo: "passageiro" | "motorista"
): Promise<void> {
  try {
    const userRef = doc(firestore, "users", uid);
    await setDoc(
      userRef,
      { modoAtual: modo, updatedAt: new Date() },
      { merge: true }
    );
    logger.success("USER_SERVICE", "modoAtual salvo no Firestore", {
      uid,
      modo,
    });
  } catch (error) {
...
export async function signInWithPhoneCredential(
  ...
  try {
    ...
    logger.success("USER_SERVICE", "Assinado com telefone (credential)", {
    ...
    logger.error(
      "USER_SERVICE",
      "Erro ao assinar com credential de telefone",
      error
    );
    throw error;
  }
...
export async function linkEmailToCurrentUser(
  ...
  try {
    if (!auth.currentUser)
      throw new Error("Usuário não autenticado para vincular email");
    const credential = EmailAuthProvider.credential(email, password);
    const linked = await linkWithCredential(
      auth.currentUser,
      credential as any
    );
    logger.success("USER_SERVICE", "Email vinculado à conta atual", {
      uid: linked.user.uid,
      email,
    });
    return linked;
...
export async function mergeUserAccounts(
  ...
  try {
    ...
    await setDoc(
      sourceRef,
      { mergedTo: targetUid, mergedAt: new Date() },
      { merge: true }
    );

    logger.success("USER_SERVICE", "Perfis mesclados com sucesso", {
      sourceUid,
      targetUid,
    });
    ...
    if (transferRides) {
      try {
        ...
        logger.success(
          "USER_SERVICE",
          "Corridas transferidas para o novo UID",
          { sourceUid, targetUid }
        );
      } catch (e) {
        logger.warn(
          "USER_SERVICE",
          "Falha ao transferir corridas durante merge (continuando)",
          e
        );
      }
    }
 ...
 */
export async function saveMotoristaRecord(
  uid: string,
  vehicleData: VehicleData
): Promise<void> {
  try {
    logger.info("USER_SERVICE", "Salvando registro em /motoristas", {
      uid,
      placa: vehicleData.placa,
    });
    ...
    logger.success("USER_SERVICE", "Registro de motorista salvo com sucesso", {
      uid,
    });
  } catch (error) {
...
export async function uploadVehicleDocument(
  ...
  try {
    logger.info(
      "USER_SERVICE",
      "Iniciando upload de documento do veículo para Firebase Storage",
      { uid, placa }
    );
    ...
    const maxSize = 10 * 1024 * 1024; // 10MB para documentos
    if (blob.size > maxSize)
      throw new Error("O documento é muito grande. Máx 10MB");

    const sanitizedPlaca = placa
      ? placa.replace(/[^a-zA-Z0-9]/g, "_").toUpperCase()
      : `doc_${Date.now()}`;
    ...
    logger.success(
      "USER_SERVICE",
      "Documento do veículo enviado para Storage",
      { uid, path: snapshot.metadata.fullPath }
    );
    return downloadURL;
  } catch (error: any) {
    ...
    if (
      ...
      const e: any = new Error(
        "Sem permissão para fazer upload do documento do veículo. Verifique as regras de segurança do Firebase Storage."
      );
      e.code = "storage-unauthorized";
      throw e;
    }
...
/**
 * Retorna um DocumentReference apontando para users/{uid} — usado por serviços que
 * precisam manipular o documento do usuário dentro de transações.
 */
export function getUserDocRefByUid(uid: string) {
